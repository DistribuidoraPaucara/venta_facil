...
export default function ReposicionesEdit({ reposicion, almacenes }: Props) {
  const { data, setData, put, errors, processing } = useForm({
    ...
    detalles: reposicion.detalles.map((d) => ({
  ...
  useEffect(() => {
    // Aquí normalmente cargarías los productos con stock bajo desde una API
    // Por ahora usamos los que ya están en los detalles
    const productosEnDetalles = reposicion.detalles.map((d) => d.producto);
    setProductosDisponibles(productosEnDetalles);
  }, [reposicion.detalles]);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    put(route('reposiciones.update', reposicion.id));
  };

  const productoSeleccionado = productosDisponibles.find((p) => p.id === selectedProducto);
    ...
    <>
      ...
      <div className="container mx-auto py-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold">Editar Reposición</h1>
            <p className="text-gray-600">{reposicion.numero}</p>
          </div>
        </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.detalles.map((detalle, index) => {
                      const producto = productosDisponibles.find((p) => p.id === detalle.producto_id);
                      return (
