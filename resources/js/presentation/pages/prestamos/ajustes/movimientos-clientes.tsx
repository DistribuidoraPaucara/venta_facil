...
export default function MovimientosClientes() {
    ...
    const cargarMovimientos = useCallback(async () => {
        ...
        try {
            ...
            params.append('categoria', 'cliente');
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Préstamos', href: '/prestamos' }, { title: 'Movimientos de Clientes', href: '/prestamos/ajustes/movimientos/clientes' }]}>
            <Head title="Movimientos de Stock - Clientes" />

            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                {/* Header */}
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">👥 Movimientos de Stock - Clientes</h1>
                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            Registro de todos los préstamos a clientes: disponibilidad y daños
                        </p>
                    </div>
                </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
                                ...
                                <th className="px-4 py-3 text-center font-semibold text-slate-900 dark:text-slate-100">Prestamo Antes</th>
                                <th className="px-4 py-3 text-center font-semibold text-slate-900 dark:text-slate-100">Cantidad</th>
                                <th className="px-4 py-3 text-center font-semibold text-slate-900 dark:text-slate-100">❌ Dañada Registrada</th>
                                <th className="px-4 py-3 text-center font-semibold text-slate-900 dark:text-slate-100">Disponible Después</th>
                                <th className="px-4 py-3 text-center font-semibold text-slate-900 dark:text-slate-100">Prestamo Después</th>
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 text-center text-sm">
                                                <span className="font-medium text-purple-700 dark:text-purple-400">
                                                    {movimiento.prestamo_cliente_anterior}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-4 py-3 text-center text-sm">
                                                <span className="font-medium text-purple-700 dark:text-purple-400">
                                                    {movimiento.prestamo_cliente_posterior}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-center bg-red-50 dark:bg-red-900/10 text-sm">
                                                <span className="font-bold text-red-700 dark:text-red-400">
                                                    {movimiento.cantidad_cliente_dañada_posterior}
                                                </span>
                                            </td>
                                        </tr>
                                    );
